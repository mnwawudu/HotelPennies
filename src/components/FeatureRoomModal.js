// ✅ src/components/FeatureRoomModal.js
...
const FeatureRoomModal = ({ roomId, onClose, onSuccess }) => {
  const [selectedType, setSelectedType] = useState('');
  const [selectedDuration, setSelectedDuration] = useState('');
  const [showPaymentModal, setShowPaymentModal] = useState(false);
  const [vendor, setVendor] = useState(null);

  // Live pricing from admin (merged over defaults)
  const [pricing, setPricing] = useState(FEATURE_PRICING);
  const [loadingPrices, setLoadingPrices] = useState(true);

  // ✅ Fetch live admin pricing (local/global × duration)
  useEffect(() => {
    const fetchPricing = async () => {
      try {
        ...
        for (const row of incoming) {
          ...
          const d = String(row?.duration || '').toLowerCase(); // '7d' | '1m' | '6m' | '1y'
  ...
  useEffect(() => {
    const fetchVendor = async () => {
      try {
        ...
        alert('Failed to load vendor profile. Please refresh and try again.');
      }
    };
  ...
  const handleProceedToPay = () => {
    if (!selectedType || !selectedDuration) {
      alert('Please select both feature type and duration.');
      return;
    }
    if (!vendor) {
  ...
  return (
    <div className="modal-overlay">
      <div className="modal-content">
        ...
        <h2 className="modal-title">Feature Your Room</h2>
      ...
      {/* ✅ Inject PayFeatureModal only when vendor is loaded */}
...
export default FeatureRoomModal;
