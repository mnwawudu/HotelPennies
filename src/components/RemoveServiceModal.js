...
const RemoveServiceModal = ({ onClose, onSuccess }) => {
  ...
  useEffect(() => {
    const fetchVendor = async () => {
      try {
        ...
        setVendorServices(res.data.businessTypes || []);
      } catch (err) {
  ...
  const handleRemove = async () => {
    if (!selected) return alert('Please select a service to remove');

    setLoading(true);
    try {
      await axios.put(
        '/api/vendor/services',
        { service: selected, action: 'remove' },
        {
          headers: { Authorization: `Bearer ${token}` }
        }
      );
  ...
  const existing = vendorServices
    .map(t => (typeof t === 'string' ? t.toLowerCase() : t?.serviceType?.toLowerCase?.()))
    .filter(Boolean);

  const filteredOptions = ALL_SERVICES.filter(type => existing.includes(type.toLowerCase()));
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2 style={{ marginBottom: '1rem', fontWeight: 600 }}>Remove Service</h2>

        {filteredOptions.length === 0 ? (
          <p style={{ marginBottom: 0 }}>You have no services to remove</p>
        ) : (
          <>
