            ...
            <tbody>
              {filteredAgreements.map((r) => (
                <tr key={r.vendorId || `${r.vendorEmail}-${r.contentHash}`}>
                  ...
                  <td title={r.vendorEmail || '-'}>{r.vendorEmail || '-'}</td>
                  <td title={r.businessName || '-'}>{r.businessName || '-'}</td>
                  <td style={{ textTransform: 'capitalize' }}>
                  ...
                  <td title={r.contentHash}>{shortRef(r.contentHash)}</td>
                </tr>
              ))}
            </tbody>
          </table>
