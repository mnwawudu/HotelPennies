...
const toPercent = (v) => {
  ...
  // Server stores fractions (0.15). If we get a plain percent (15),
  // leave it as-is. If it's <= 1, treat as fraction and *100.
...
export default function AdminCommissions() {
  ...
  async function fetchSettings() {
    ...
    try {
      ...
      const s = resp.data || {};
      // Convert server fractions to UI percents
      setValues((v) => ({
        ...v,
  ...
  const save = async () => {
    ...
    setOk('');
    try {
      // Send **camelCase** keys the server expects.
      // Server accepts percents (e.g., 17) and converts to fractions internally.
      const payload = {
  ...
  const resetToEnv = async () => {
    ...
    setOk('');
    try {
      // optional endpoint — if not present, just refetch
      try {
  ...
  return (
    <div className="admin-page" style={{ maxWidth: 720 }}>
      ...
      <p style={{ color: '#555', marginTop: -6 }}>
        Control platform %, cashback and referral for bookings. These “knobs” override the .env defaults
        and are used by the ledger at booking time.
      </p>
