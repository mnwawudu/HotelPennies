...
const crypto = require('crypto'); //  for duplicate detection
...
//  NEW: minimal additions for internal withdraw (no external provider) 
...
router.get('/dashboard', auth, async (req, res) => {
  try {
    ...
    // ≡ƒöü Mature vendor rows before calculating balances
    await releaseDueForVendor(vendorId).catch(() => {});

    // ≡ƒöÆ Compute verification flags so the banner can disappear when admin approves
    ...
    res.json({
      vendor: {
        ...
        isFullyVerified: computedIsFullyVerified, // Γ£à banner logic now correct
...
router.get('/profile', auth, async (req, res) => {
  try {
    ...
    // ≡ƒæç computed flag: unlock if admin-approved OR docs complete OR old flag already true
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // ≡ƒöÆ Reject duplicates within the same submission (same bytes)
...
//  REPLACED: INTERNAL withdraw (no external provider calls) 
